...
osThreadId_t osThreadNew (osThreadFunc_t func, void *argument, const osThreadAttr_t *attr) {
  ...
  if ((IRQ_Context() == 0U) && (func != NULL)) {
    ...
    if (mem == 1) {
        ...
        hTask = xTaskCreateStatic ((TaskFunction_t)func, name, stack, argument, (prio - (UBaseType_t)osPriorityIdle), (StackType_t  *)attr->stack_mem, /* FreeRTOS priorities start from 0, remap osPriority_t to match FreeRTOS priority indexes */
      ...
      if (mem == 0) {
          ...
          if (xTaskCreate ((TaskFunction_t)func, name, (configSTACK_DEPTH_TYPE)stack, argument, (prio - (UBaseType_t)osPriorityIdle), &hTask) != pdPASS) { /* FreeRTOS priorities start from 0, remap osPriority_t to match FreeRTOS priority indexes */
          /*^^^ END OF PATCH STMC-485 ^^^*/
            hTask = NULL;
          }
        #endif
...
osStatus_t osThreadSetPriority (osThreadId_t thread_id, osPriority_t priority) {
  ...
  if (IRQ_Context() != 0U) {
    ...
    vTaskPrioritySet (hTask, ((UBaseType_t)priority - (UBaseType_t)osPriorityIdle)); /* FreeRTOS priorities start from 0, remap osPriority_t to match FreeRTOS priority indexes */
    /*^^^ END OF PATCH STMC-485 ^^^*/
  }
...
osPriority_t osThreadGetPriority (osThreadId_t thread_id) {
  ...
  if ((IRQ_Context() != 0U) || (hTask == NULL)) {
    ...
    prio = (osPriority_t)(uxTaskPriorityGet (hTask) + (BaseType_t)osPriorityIdle); /* FreeRTOS priorities start from 0, remap to osPriority_t by adding the value of osPriorityIdle */
    /*^^^ END OF PATCH STMC-485 ^^^*/
  }
