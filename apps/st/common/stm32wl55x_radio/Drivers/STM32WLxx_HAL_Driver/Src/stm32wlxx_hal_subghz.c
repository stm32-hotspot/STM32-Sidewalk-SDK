...
#define SUBGHZ_RFBUSY_LOOP_TIME    ((SystemCoreClock*24U)>>16U)
...
HAL_StatusTypeDef HAL_SUBGHZ_Init(SUBGHZ_HandleTypeDef *hsubghz)
{
      ...
      (subghz_state == HAL_SUBGHZ_STATE_RESET_RF_READY))
  {
    ...
    /* Enable falling edge trigger in EXTI for BUSY line */
    LL_EXTI_EnableFallingTrig_32_63(LL_EXTI_LINE_45);
...
HAL_StatusTypeDef HAL_SUBGHZ_DeInit(SUBGHZ_HandleTypeDef *hsubghz)
{
  ...
  /* Disable EXTI 44-45 : Radio IRQ ITs for CPU2 */
  LL_C2_EXTI_DisableIT_32_63(LL_EXTI_LINE_44 | LL_EXTI_LINE_45);
  /*^^^ END OF WORKAROUND FOR STMC-495 ^^^*/
  ...
  /* Disable EXTI 44-45 : Radio IRQ ITs for CPU1 */
  LL_EXTI_DisableIT_32_63(LL_EXTI_LINE_44 | LL_EXTI_LINE_45);
  /*^^^ END OF WORKAROUND FOR STMC-495 ^^^*/
  ...
  /* Disable falling edge trigger in EXTI */
  LL_EXTI_DisableFallingTrig_32_63(LL_EXTI_LINE_45);

  /* Clear BUSY flag in EXTI */
  LL_EXTI_ClearFlag_32_63(LL_EXTI_LINE_45);
  /*^^^ END OF WORKAROUND FOR STMC-495 ^^^*/
                                            ...
                                            uint16_t Size)
{
  ...
  if (hsubghz->State == HAL_SUBGHZ_STATE_READY)
  {
    ...
    /*vvv WORKAROUND FOR STMC-495 vvvvvvvvvv*/
    /* Note: this operation does not trigger the BUSY indication */
                                        ...
                                        uint16_t Size)
{
  ...
  if (hsubghz->State == HAL_SUBGHZ_STATE_READY)
  {
    ...
    if ((Command != RADIO_SET_SLEEP) && (Command != RADIO_CFG_DIOIRQ) && (Command != RADIO_CLR_IRQSTATUS) && (Command != RADIO_CLR_ERROR))
    {
      /* Capture the current state of Sub-GHz radio IRQ in NVIC */
      irqWasEnabled = NVIC_GetEnableIRQ(SUBGHZ_Radio_IRQn);

      /* Disable IRQ in NVIC to avoid false triggers by the BUSY line (EXTI45) */
      HAL_NVIC_DisableIRQ(SUBGHZ_Radio_IRQn);

      /* Clear EXTI pending flag to arm the capture of the falling edge on the BUSY line */
      LL_EXTI_ClearFlag_32_63(LL_EXTI_LINE_45);

      /* Enable IRQ indication in EXTI - without this step EXTI won't monitor the edge transitions */
#if defined(CORE_CM0PLUS)
      /* Enable EXTI 45 : Radio IRQ ITs for CPU2 */
      LL_C2_EXTI_EnableIT_32_63(LL_EXTI_LINE_45);
#else
      /* Enable EXTI 45 : Radio IRQ ITs for CPU1 */
      LL_EXTI_EnableIT_32_63(LL_EXTI_LINE_45);
#endif /* CORE_CM0PLUS */
    }
    /*^^^ END OF WORKAROUND FOR STMC-495 ^^^*/
...
#if defined(CORE_CM0PLUS)
      /* Disable EXTI 45 : Radio IRQ ITs for CPU2 */
      LL_C2_EXTI_DisableIT_32_63(LL_EXTI_LINE_45);
#else
      /* Disable EXTI 45 : Radio IRQ ITs for CPU1 */
      LL_EXTI_DisableIT_32_63(LL_EXTI_LINE_45);
#endif /* CORE_CM0PLUS */

      /* Clear EXTI pending flag to avoid false-positive triggers of Sub-GHz radio IRQ in NVIC */
      LL_EXTI_ClearFlag_32_63(LL_EXTI_LINE_45);

      /* Clear pending IRQ in NVIC because it might have been triggered by EXTI45. If it was triggered by the real radio IRQ, it will be set again immediately */
      HAL_NVIC_ClearPendingIRQ(SUBGHZ_Radio_IRQn);

      /* Restore IRQ state in NVIC if it was modified by us */
      if (irqWasEnabled != 0u)
      {
...
HAL_StatusTypeDef SUBGHZ_WaitOnBusy(SUBGHZ_HandleTypeDef *hsubghz)
{
  ...
  /* Wait either until EXTI indicates a falling edge on the BUSY line (indicating the end of wait period) or while the BUSY line remains high (indicating the radio is processing the data) */
  while ((LL_EXTI_IsActiveFlag_32_63(LL_EXTI_LINE_45) == 0u) || (LL_PWR_IsActiveFlag_RFBUSYS() != 0u))
  {
    if (count == 0U)
    {
      status  = HAL_ERROR;
      hsubghz->ErrorCode = HAL_SUBGHZ_ERROR_RF_BUSY;
      return status; /* Terminate from here */
    }
    count--;
  }
  /*^^^ END OF WORKAROUND FOR STMC-495 ^^^*/
