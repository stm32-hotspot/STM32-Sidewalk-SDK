...
void BLE_TIMER_Init(void)
{
  ...
  /**
   * Semaphore should be created before the task that's using it is created, otherwise BLE_TIMER_Task may immediately after creation if
   * it has higher priority than BLE_TIMER_Init() caller. As a result, enter BLE_TIMER_Task may enter a deadlock or cause a HardFault by
   * accessing null semaphore pointer
   */
...
/*vvv PATCH STMC-485 vvvvvvvvvv*/
uint8_t BLE_TIMER_Deinit(void)
{
  osStatus_t  status;

  if ((BLE_TIMER_List.next != NULL) && (BLE_TIMER_List.prev != NULL))
  {
    while (LST_is_empty(&BLE_TIMER_List) == FALSE)
    {
      BLE_TIMER_t* currentNode = (BLE_TIMER_t*)BLE_TIMER_List.next;
      BLE_TIMER_Stop(currentNode->id);
    }
  }

  if (BleTimerTaskHandle != NULL)
  {
    status = osThreadTerminate(BleTimerTaskHandle);
    if(status != osOK)
    {
      LOG_ERROR_APP( "BLE Timer task termination failed");
      return BLE_STATUS_FAILED;
    }
  ...
  {
    status = osSemaphoreDelete(BleTimerSemaphore);
    if(status != osOK)
    {
      LOG_ERROR_APP( "BLE Timer semaphore deletion failed");
      return BLE_STATUS_FAILED;
    }
