...
/**
  * @brief Get rtc time
  * @param[out] subSeconds in us
  * @return time seconds
  */
uint32_t TIMER_IF_GetTimeUs(uint32_t * const uSeconds);

/**
  * @brief converts time in us to time in ticks
  * @param[in] uSeconds time in microseconds
  * @return time in timer ticks
  */
uint64_t TIMER_IF_Convert_us2Tick(const uint64_t uSeconds);

/**
  * @brief converts time in ticks to time in us
  * @param[in] tick time in timer ticks
  * @return time in timer microseconds
  */
uint64_t TIMER_IF_Convert_Tick2us(const uint64_t ticks);
