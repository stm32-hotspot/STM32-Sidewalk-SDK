...
typedef enum
{
  ...
  /* Since the enum is now used to index an array, the values of the enum items should be defined explicitly.
   * As per C spec, enum only guarantees the automatically assigned values to be incrementing, but they are not
   * guaranteed to start from zero, nor they are guaranteed to always increment by 1.
   */
  // UTIL_LPM_SLEEPMODE,
  // UTIL_LPM_STOPMODE,
  // UTIL_LPM_OFFMODE,
  UTIL_LPM_SLEEPMODE = 0,
  UTIL_LPM_STOPMODE = 1,
  UTIL_LPM_OFFMODE = 2,
